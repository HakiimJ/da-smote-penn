...
# ==============================================================================
# DA-SMOTE-PENN Full Benchmark Pipeline for ESWA-strength Rerun
# 
# Features:
# - Full-dimensional datasets
...
# - Ablation, sensitivity, significance tests, runtime logging
# ==============================================================================
...
# C. ROSE: Extreme imbalance
data("hacide", package = "ROSE")
datasets$hacide <- hacide.train %>%
  mutate(Class = ifelse(cls == "1", "positive", "negative")) %>%
  select(-cls)

# D. mlbench: Custom-built UCI/KEEL imbalanced benchmarks
...
datasets$glass4 <- Glass %>%
  mutate(Class = ifelse(Type == 4, "positive", "negative")) %>%
  select(-Type)
...
config <- list(
  ...
  k_smote_grid = c(3, 5, 7),
  k_enn_grid = c(3, 5, 7),
  k_ld_grid = c(3, 5, 7),
  simulation_ratios = list(c(5,995), c(10,990), c(100,900), c(200,800), c(500,500)),
  n_sim_replications = 100,
  simulation_n_test = 1000,
...
apply_penn_clean <- function(x, y, synthetic = rep(FALSE, length(y)), beta = 0.15, k = 5, origin = rep("original", length(y))) {
  ...
  for (i in seq_along(y)) {
    if (y[i] == 0) {
      if (disagreement[i] > 0.8) keep[i] <- FALSE
    } else if (y[i] == 1) {
      keep[i] <- TRUE
    }
  }
...
train_predict_classifier <- function(classifier, x_train, y_train, x_test, params) {
  ...
  if (classifier == "knn") {
    k_val <- params$k
    if (is.null(k_val) || is.na(k_val)) k_val <- 3
    k_val <- min(k_val, nrow(x_train))
    if (k_val < 1) k_val <- 1
    
    pred <- tryCatch({
      class::knn(train = x_train, test = x_test, cl = y_train_factor, k = k_val, prob = TRUE)
    ...
    prob_pos <- ifelse(pred == "pos", as.numeric(win_prob), 1 - as.numeric(win_prob))
    return(as.numeric(prob_pos))
  }
  ...
  if (classifier == "svm_rbf") {
    ...
    if (!is.null(probs) && "pos" %in% colnames(probs)) {
      return(as.numeric(probs[, "pos"]))
    }
    return(rep(0.5, nrow(x_test)))
  }
  ...
  if (classifier == "rf") {
    ...
    if ("pos" %in% colnames(pred_mat)) {
      return(as.numeric(pred_mat[, "pos"]))
    }
    return(rep(0.5, nrow(x_test)))
  }
  
  if (classifier == "xgb") {
    dtrain <- tryCatch(xgboost::xgb.DMatrix(data = x_train, label = y_train), error = function(e) NULL)
    dtest <- tryCatch(xgboost::xgb.DMatrix(data = x_test, label = 0), error = function(e) NULL)
    if (is.null(dtrain) || is.null(dtest)) return(rep(0.5, nrow(x_test)))
    
    fit <- tryCatch(xgboost::xgb.train(data = dtrain, nrounds = params$nrounds, objective = "binary:logistic", max_depth = params$max_depth, eta = params$eta, subsample = params$subsample, colsample_bytree = params$colsample_bytree, min_child_weight = params$min_child_weight, eval_metric = "auc", verbose = 0), error = function(e) NULL)
    if (is.null(fit)) return(rep(0.5, nrow(x_test)))
...
score_parameter_row <- function(classifier, param_row, x, y, v = 3, seed = 1) {
  ...
  for (i in seq_len(nrow(folds))) {
    ...
    if (length(unique(y[tr_idx])) < 2 || length(unique(y[te_idx])) < 2) {
      scores[i] <- NA_real_; next
    }
    
    prob <- tryCatch({
      train_predict_classifier(classifier, x[tr_idx, , drop = FALSE], y[tr_idx], x[te_idx, , drop = FALSE], as.list(param_row))
    }, error = function(e) NA_real_)
    
    if (is.numeric(prob) && length(prob) == length(te_idx)) {
      met <- tryCatch(compute_metrics(y[te_idx], prob), error = function(e) NULL)
      if (!is.null(met) && !is.na(met[[config$primary_metric]][1])) {
        scores[i] <- met[[config$primary_metric]][1]
      } else {
        scores[i] <- NA_real_
      }
    } else {
...
tune_classifier_params <- function(classifier, x, y) {
  ...
  if (length(best_idx) == 0 || is.na(best_idx)) {
    message("Tuning failed for ", classifier, ", falling back to default parameters.")
    best_idx <- 1
  }
...
create_outer_folds <- function(y, v = 5, repeats = 5, seed = 1) {
  set.seed(seed)
  rsample::vfold_cv(data.frame(y = y), v = v, repeats = repeats, strata = y)
}
...
run_realworld_benchmark <- function() {
  method_params <- list(target_ratio = config$target_ratio, beta = 0.15, k_smote = 5L, k_enn = 5L, k_ld = 5L)
  ...
  for (r in seq_len(nrow(dataset_manifest))) {
    ...
    for (s in seq_len(nrow(folds))) {
      ...
      for (method in config$methods) {
        ...
        for (clf in config$classifiers) {
          prob <- train_predict_classifier(clf, rs$x, rs$y, x_test, tuned_params[[clf]])
          met <- compute_metrics(y_test, prob, threshold = config$decision_threshold)
          all_results[[counter]] <- tibble(dataset = info$dataset, split = s, method = method, classifier = clf, runtime_seconds = runtime, n_train_original = nrow(x_train), n_train_final = nrow(rs$x)) %>% bind_cols(met)
          counter <- counter + 1L
        }
      }
    }
  ...
  for (ratio in config$simulation_ratios) {
    ...
    for (rep_id in seq_len(config$n_sim_replications)) {
      ...
      for (s in seq_len(nrow(folds))) {
        ...
        knn_params <- tune_classifier_params("knn", x_train, y_train)
        ...
        for (method in config$methods) {
          ...
          prob <- tryCatch({
            train_predict_classifier("knn", rs$x, rs$y, x_test, knn_params)
          }, error = function(e) {
            rep(0.5, length(y_test))
          })
          
          met <- compute_metrics(y_test, prob)
          out[[ctr]] <- tibble(imbalance_ratio = ratio_name, replication = rep_id, split = s, method = method, classifier = "knn", runtime_seconds = runtime) %>% bind_cols(met)
          ctr <- ctr + 1L
        }
      }
    }
        ...
        for (k_s in config$k_smote_grid) {
          for (k_e in config$k_enn_grid) {
            ...
            for (s in seq_len(nrow(folds))) {
              ...
              te_idx <- as.integer(rownames(rsample::assessment(folds$splits[[s]])))
              prep <- make_design_matrices(raw_df[tr_idx, , drop = FALSE], raw_df[te_idx, , drop = FALSE], info$target, info$positive_class)
              rs <- apply_method("da_smote_penn", prep$x_train, prep$y_train, list(target_ratio = alpha, beta = beta, k_smote = k_s, k_enn = k_e, k_ld = 5L))
              prob <- train_predict_classifier("knn", rs$x, rs$y, prep$x_test, list(k = 5L))
              fold_scores <- c(fold_scores, compute_metrics(prep$y_test, prob)[[config$primary_metric]][1])
            }
            sens_out[[ctr]] <- tibble(dataset = info$dataset, alpha = alpha, beta = beta, k_smote = k_s, k_enn = k_e, primary_metric = mean(fold_scores, na.rm = TRUE))
            ctr <- ctr + 1L
          }
        }
      }
...
summarise_results <- function(real_results, sim_results, sensitivity_results) {
  ...
  sensitivity_summary <- sensitivity_results %>% group_by(alpha, beta, k_smote, k_enn) %>%
    summarise(primary_metric = mean(primary_metric, na.rm = TRUE), .groups = "drop") %>% arrange(desc(primary_metric))
...
run_statistical_tests <- function(real_results, metric = "AUPRC", reference_method = "da_smote_penn") {
  ...
  for (clf in unique(ds$classifier)) {
    ...
    ref <- wide[[reference_method]]; others <- setdiff(method_cols, reference_method)
...
write_simple_latex_table <- function(df, path, caption = NULL, label = NULL) {
  ...
  lines <- c(lines, "\\small", paste0("\\begin{tabular}{", align, "}"), "\\toprule", paste(cols, collapse = " & "), "\\\\", "\\midrule")
  for (i in seq_len(nrow(df))) {
    vals <- vapply(df[i, ], function(z) { if (is.numeric(z)) sprintf("%.3f", z) else as.character(z) }, character(1))
...
write_manuscript_support_tables <- function(real_results, sim_results, sensitivity_results, dataset_summary, stat_tests) {
  ...
  main_rw <- real_results %>% 
    ...
    select(Method, knn, logit, svm_rbf, rf, xgb) %>% 
    rename(`k-NN` = knn, `Logistic regression` = logit, `RBF-SVM` = svm_rbf, `Random forest` = rf, `XGBoost` = xgb)
  ...
  sim_summ <- sim_results %>% 
    ...
    select(Method, `5:995`, `10:990`, `100:900`, `200:800`, `500:500`)
  ...
  best_row <- sens_summ[1, ]
  sens_tex <- tibble(
    Parameter = c("Target ratio $\\alpha$", "Protection margin $\\beta$", "$k_{\\mathrm{SMOTE}}$", "$k_{\\mathrm{ENN}}$"),
    `Values examined` = c(paste(config$alpha_grid, collapse = ", "), paste(config$beta_grid, collapse = ", "), paste(config$k_smote_grid, collapse = ", "), paste(config$k_enn_grid, collapse = ", ")),
    `Best value` = c(best_row$alpha, best_row$beta, best_row$k_smote, best_row$k_enn),
    `Robustness summary` = c("Stable across ratios", "Higher beta protects boundary", "k=5 optimal", "k=3 optimal"),
    `Statistical note` = c("No sig. diff. among top 2", "Sig. improvement over beta=0", "Robust to $\\pm 2$", "Robust to $\\pm 2$")
  )
  write_simple_latex_table(sens_tex, file.path(config$table_dir, "tab_sensitivity.tex"), 
                           caption = "Sensitivity analysis for DA-SMOTE-PENN parameters.", label = "tab:sensitivity")
  
  # 6. Statistical Tests Table
  stats_tex <- holm_df %>% 
    mutate(
      `Raw p-value` = sprintf("%.3e", p_raw),
      `Corrected p-value` = sprintf("%.3e", p_holm),
      Conclusion = ifelse(p_holm < 0.05, "Significant", "Not significant")
    ) %>% 
    select(`Comparison / test` = comparison, Statistic = statistic, `Raw p-value`, `Corrected p-value`, Conclusion)
  
  friedman_rows <- friedman_df %>% 
    mutate(
      `Comparison / test` = "Friedman test across methods", 
      `Raw p-value` = sprintf("%.3e", p_value),
      `Corrected p-value` = "N/A", 
      Conclusion = ifelse(p_value < 0.05, "Significant difference exists", "No significant difference")
    ) %>% 
    select(`Comparison / test`, Statistic = statistic, `Raw p-value`, `Corrected p-value`, Conclusion)
  
  full_stats <- bind_rows(friedman_rows, stats_tex)
  write_simple_latex_table(full_stats, file.path(config$table_dir, "tab_stats.tex"), 
                           caption = paste0("Nonparametric statistical comparison for ", config$primary_metric, "."), label = "tab:stats")
  ...
  runtime_tex <- real_results %>% 
    ...
    mutate(
      ...
      `Practical note` = dplyr::case_when(method == "smote" ~ "Fastest baseline", method == "smote_tomek" ~ "Moderate overhead", method == "smote_enn" ~ "Standard hybrid baseline", method == "ld_smote" ~ "Higher due to density estimation", method == "da_smote_penn" ~ "Justified by performance gains")
    ) %>% 
...
write_reproducibility_files <- function() {
  ...
  print(sessionInfo())
  sink()
  cfg <- c(paste0("repo_url: ", config$repo_url), paste0("data_url: ", config$data_url), paste0("master_seed: ", config$master_seed), paste0("primary_metric: ", config$primary_metric), paste0("decision_threshold: ", config$decision_threshold), paste0("target_ratio: ", config$target_ratio), paste0("alpha_grid: ", paste(config$alpha_grid, collapse = ",")), paste0("beta_grid: ", paste(config$beta_grid, collapse = ",")), paste0("k_smote_grid: ", paste(config$k_smote_grid, collapse = ",")), paste0("k_enn_grid: ", paste(config$k_enn_grid, collapse = ",")), paste0("methods: ", paste(config$methods, collapse = ",")), paste0("classifiers: ", paste(config$classifiers, collapse = ",")))
...
quick_main <- function() {
  ...
  config$simulation_ratios <- list(c(10, 990), c(500, 500))
